...
<Steps>
  <Step title="Create an account">
    Go to [dashboard.privataswap.com/signup](https://dashboard.privataswap.com/signup). Enter your business email and choose a password (minimum 12 characters), or request a magic-link to your email instead.
...
For enterprise organizations with 10+ members, enterprise SSO (SAML 2.0 / OIDC) can be configured at **Team → SSO** using the WorkOS connection wizard. Contact [partners@privataswap.com](mailto:partners@privataswap.com) to enable.

---

## Audit log

Every action taken in the dashboard — key creation, role changes, payout requests, webhook replays — is recorded in the **Audit Log** with the actor's identity, IP address, and timestamp. Logs are tamper-evident (hash-chained) and retained for 2 years. Export as NDJSON or CSV for compliance reporting.
